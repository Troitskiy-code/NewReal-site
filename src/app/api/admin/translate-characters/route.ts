...
function missingSourceFields(character: CharacterRow): CharacterSourceField[] {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const pending = characters.filter((character) => missingSourceFields(character).length > 0);
    ...
    await mapPool(pending, CONCURRENCY, async (character, index) => {
