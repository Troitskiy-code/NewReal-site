...
function formatDuration(ms: number, t: (key: string, options?: Record<string, number>) => string): string {
  if (ms <= 0) return t("coins.min");
  const hours = Math.floor(ms / (60 * 60 * 1000));
  const minutes = Math.floor((ms % (60 * 60 * 1000)) / (60 * 1000));
  if (hours > 0) return t("coins.hoursMinutes", { hours, minutes });
  return t("coins.minutes", { minutes });
}
    ...
    <div className="flex min-h-dvh flex-col overflow-hidden bg-wd-bg text-wd-text">
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-10 overflow-y-auto px-4 py-8 scrollbar-subtle sm:px-6 lg:px-8">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl font-black tracking-tight text-white">{t("coins.title")}</h1>
          ...
          <>
            <section className="wd-card p-8 text-center">
              ...
              <p className="mt-3 flex items-center justify-center gap-3 text-5xl font-black text-white">
                ...
                {formatCoins(balance.verseCoins, i18n.language)} VC
              </p>
            </section>
            ...
            {balance.subscriptionActive && balance.subscriptionLabel && balance.subscriptionEnd && (
              <section className="wd-card flex items-start gap-3 border-wd-secondary/30 p-5">
                ...
                <div>
                  <p className="text-sm font-bold text-white">{t("coins.yourSubscription", { label: balance.subscriptionLabel })}</p>
                  <p className="mt-1 text-xs text-wd-text-secondary">
                    {t("coins.validUntil", { date: formatDate(balance.subscriptionEnd, i18n.language) })}
                  </p>
                </div>
              </section>
            )}
            ...
            <section className="wd-card space-y-5 p-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs text-wd-text-secondary">
                  <span>{t("coins.streakDay", { day: streakDay })}</span>
                  <span>
                    {t("coins.today")}
              ...
              {!balance.canClaimBonus && (
                <div className="rounded-wd border border-wd-border bg-[#0A0A0A] p-3 text-xs text-wd-text-secondary">
                  <p className="font-semibold text-white">{t("coins.alreadyClaimed")}</p>
                  <p className="mt-1">
                    {t("coins.nextBonus")}{" "}
                    <span className="font-bold text-wd-secondary">
                      {formatDuration(balance.msUntilNextBonus, t)}
                    </span>{" "}
              ...
              >
                ...
                {claiming
                  ? t("coins.claiming")
                  : balance.canClaimBonus
                    ? t("coins.claim", { amount: balance.currentBonusAmount })
                    : t("coins.unavailable")}
              </button>
            </section>
          </>
